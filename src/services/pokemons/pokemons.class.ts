...
export class Pokemons implements ServiceMethods<Data> {
  ...
  async find(params?: Params): Promise<Array<Pokemon>> {
    ...
    for (const pokemon of allPokemons) {
      ...
      const diamondGame = speciesDetail["flavor_text_entries"].find(
        (item: any) => item.version.name === "diamond"
      );
      const pokedexService: any = this.app.service("pokedex");
      const pokedex = await pokedexService.find({
        query: { pokemonId: pokemonDetail.id, favourite: true },
      });
  ...
  async get(id: Id, params?: Params): Promise<Data> {
    ...
    const res: Array<Pokemon> = await pokemonService.find();
    const pokemons = res.filter((item) => item.id === Number(id));
    if (pokemons.length > 0) {
      const pokedexService: any = this.app.service("pokedex");
      const pokedex = await pokedexService.find({
        query: { pokemonId: id, favourite: true, user: params?.user?.email },
      });
      return {
        ...pokemons[0],
        ...(pokedex.total > 0 && { favourite: pokedex.data[0].favourite }),
      };
    } else {
      return {};
    }
  ...
  async patch(id: NullableId, data: Pokedex, params?: Params): Promise<Data> {
    if (!id && !data.id && data.pokemonId)
      throw new BadRequest("pokemonId is required");

    const itemId: number =
      Number(id) || Number(data.id) || Number(data.pokemonId);
    const pokedexService: any = this.app.service("pokedex");
    const pokedex = await pokedexService.find({
      query: { pokemonId: itemId, user: params?.user?.email, $limit: 1 },
    });
