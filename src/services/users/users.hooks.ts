...
const validateUserData = async (
  ...
  const usernameReqex = /^(?!.*\s)(?=.*[a-zA-Z])(?=.*[-._]).{1,8}$/;
  ...
  if (!usernameReqex.test(data.username) ) throw new BadRequest('username should be between 1 - 8 characters and only allows latin letters, digits and special characters (-, ., _)');
